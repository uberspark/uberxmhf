...
*
  --enable-debug-event-logger, enables eveng logger, which prints number of
  events (e.g. intercepts) happend on each CPU regularly in YAML format
...
*
  --enable-skip-runtime-bss, reduces size of image generated for XMHF runtime,
  but can only be enabled when DRT is disabled

*
  --enable-skip-init-smp, skip waking up APs in XMHF bootloader, reduces boot
  time, but can only be enabled when DRT is disabled

*
  --enable-skip-bootloader-hash, skip computing hashes in XMHF bootloader,
  reduces boot time, but can only be enabled when DRT is disabled
