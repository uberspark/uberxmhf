...
3. Ensure that your reconfigure prior to building (this is required to 
   rewrite ``xmhf-uobjs/UOBJLST`` that is used by the ``Makefile``). Following the 
   steps in  :doc:`uberXMHF (pc-intel-x86_32) Veryfying and Building </pc-intel-x86_32/verify-build.html>`
