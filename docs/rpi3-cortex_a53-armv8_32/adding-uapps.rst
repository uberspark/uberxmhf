...
1.  Create directory for |uberapp| functionality (e.g., `uapp_newapp`) within `uapps` directory
    ...
    * Place header files in `include` as `newapp.h`. This header defines any data structures used for 
      passing data, and a |uberapp| ID (`#define UAPP_NEWAPP_ACTION`)
...
3.  Add a test program that exercises your hypapp in `rgapps/linux`
    ...
    * In userspace, this test program will need to include `uhcall.h`, where the |uberapp| is called 
      using `uhcall(UAPP_NEWAPP_ACTION, buf_ptr, sizeof(buf))`
...
4.  Running userspace test programs.
    ...
    * Test program can now invoke |uberapp|
