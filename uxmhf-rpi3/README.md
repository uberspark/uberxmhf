...
5. Obtain boot-partition extents
	a. sudo fdisk -l /dev/mmcblk0
	b. BP_START_SECTOR = note down start sector of /dev/mmcblk0p0 in the output
	c. BP_END_SECTOR = note down end sector of /dev/mmcblk0p0 in the output
	
6. Build uxmhf-rpi3 on development system
	a. git clone https://gitlab.com/hypcode/projs/sei-cdra/prototypes/uxmhf-rpi3.git
	b. cd uxmhf-rpi3/uxmhf-rpi3
	c. ./bsconfigure.sh
	d. ./configure --with-boot-partition-start=BP_START_SECTOR --with-boot-partition-end=BP_END_SECTOR 
	e. make clean
	f. make OSKRNLIMG=~/uxmhf-rpi3-staging/kernel7.img
	g. cp uxmhf-rpi3.img ~/uxmhf-rpi3-staging/.
	h. cp rpi3-config.txt ~/uxmhf-rpi3-staging/config.txt

7. Building uhcallkmod on development system
...
8. Building uhcalltest on development system
	b. Cd rgapps/linux
	c. Make -w all
...
9. Deploying on sd-card
	...
	h. sudo cp ~/uxmhf-rpi3-staging/config.txt ~/mnt/pi-boot/.
	i. append loglevel=8 memblock=debug dwc_otg.fiq_enable=0 dwc_otg.fiq_fsm_enable=0 to ~/mnt/pi-boot/cmdline.txt
	
	j. sudo cp ~/uxmhf-rpi3-staging/boot/* ~/mnt/pi-boot/.
	k. sudo mkdir -p ~/mnt/pi-boot/overlays
	l. sudo cp -R ~/uxmhf-rpi3-staging/overlays/* ~/mnt/pi-boot/overlays/.
	m. sudo cp ~/uxmhf-rpi3-staging/kernel7.img ~/mnt/pi-boot/.
	n. sudo mkdir -p ~/mnt/pi-root/lib/firmware
	o. sudo cp -R ~/uxmhf-rpi3-staging/mod_install/lib/firmware/* ~/mnt/pi-root/lib/firmware/.
	p. sudo mkdir -p ~/mnt/pi-root/lib/modules/4.4.50-v7+
	q. sudo cp -R ~/uxmhf-rpi3-staging/mod_install/lib/modules/4.4.50-v7+/* ~/mnt/pi-root/lib/modules/4.4.50-v7+/.
	r. edit ~/mnt/pi-root/etc/fstab and comment out line beginning with /dev/mmcblk0p1 which is mounted to boot

10. Setup serial debugging
