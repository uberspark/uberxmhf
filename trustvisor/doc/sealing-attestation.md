h1. Sealing and Attestation

{{toc}}

Last updated 2011.08.29 by Jon.

h2. TrustVisor support for Micro-TPM Sealing: Initialization and Steady-State
...
** A prefix of 0x0001 is mandatory. I'm concerned that 0x00011337 and similar may provoke excessively hostile behavior.  
...
# If no index is provided or the provided index does not point to a defined space, *goto Handle-Failure*. 
...
h3. Convenience / Streamlining feature possibilities for the future

** TrustVisor doesn't know the TPM's owner secret so it cannot create the relevant NVRAM index on its own.
*** If no NV Index is defined (or if it is defined incorrectly), development can proceed and PALs using Micro-TPM sealed storage will work, but sealed data will become irretrievable after a platform reboot.
