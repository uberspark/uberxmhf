...
make clean
make > 1.txt
objdump -D ~/httpd-2.2.14/.libs/httpd > httpd.D
echo \#define MAGIC_ADDR 0x`grep -A1 "<get_addr@plt>:" httpd.D | tail -1 | cut -d x -f 2` > ~/httpd-2.2.14/modules/ssl/magic_addr.h
objdump -D /usr/local/nssl/lib/libssl.so.0.9.8 > nlib.D
echo \#define THUNK_ADDR 0x`grep "<__i686.get_pc_thunk.bx>:" nlib.D | cut -c 1-8` >> ~/httpd-2.2.14/modules/ssl/magic_addr.h
echo \#define ENTRY_ADDR 0x`grep "<sscb2>:" nlib.D | cut -c 1-8` >> ~/httpd-2.2.14/modules/ssl/magic_addr.h
cat ~/httpd-2.2.14/modules/ssl/magic_addr.h
make clean
make > 2.txt
