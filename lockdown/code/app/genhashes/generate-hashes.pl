...
# generate hashes for all EXE, DLL and SYS within system32
recurse("/cygdrive/x/windows");
# dumphashesforfile("/cygdrive/x/windows/system32/drivers/usbhub.sys");
  ...
  for my $eachFile (glob($path.'*')) {
    ...
    if( -d $eachFile) {
      ## pass the directory to the routine ( recursion )
			# if( !($eachFile =~ "dllcache") ){
				recurse($eachFile);
			# }
    } else {
      ## print the file ... tabbed for readability
      # if ( $eachFile=~/drv$/ || $eachFile=~/ocx$/ || $eachFile=~/cpl$/ ){ 
				print "\t",$eachFile,"\n";
