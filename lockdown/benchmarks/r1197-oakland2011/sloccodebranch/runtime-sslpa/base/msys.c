...
void *xrealloc(void *ptr, unsigned size){
  ...
  p = (UNIT *)( (unsigned)ptr - sizeof(UNIT) );
	...
	memcpy(dstptr, ptr, srcsize);
	xfree(ptr);
}

void *xcalloc (unsigned  n, unsigned s){
	return(xmalloc(n*s));
}

void xallocator_init()
{
     unsigned len=HEAP_SIZE;
		 len  += 3;
     len >>= 2;
     len <<= 2;
