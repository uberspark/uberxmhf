...
uint16   IINCHIP_READ_o(uint32 addr)
{
...
void     setSn_CR(SOCKET s, uint16 com)
{
	...
	//LogWr(0x31); LogWr(s); LogWr(com); LogWrTim();
  ...
  while(IINCHIP_READ(Sn_CR(s))) { // wait until Sn_CR is cleared.
  	...
  	if ( cnt>20000 ) {
	  	...
	  	//LogWr(0x774); LogWr(s); LogWr(v1); LogWr(v2); LogWr(v3);
...
uint8    getSn_IR(SOCKET s)
{
	...
	//LogWr(0x33); LogWr(s); LogWr(v1); LogWrTim();
...
void     setSn_IR(SOCKET s, uint8 ir)
{
  ...
  //LogWr(0x32); LogWr(s); LogWr(ir); LogWrTim();
}
...
uint8    getSn_SSR(SOCKET s)
{
  ...
  while(1) {
		...
		//LogWr(0x551); LogWr(s); LogWr(ssr); LogWr(ssr1); LogWr(ssr2);
		ssr2 = (uint8)IINCHIP_READ(Sn_SSR(s));
		//LogWr(ssr2);
...
uint8    pppinit_in(uint8 * id, uint8 idlen, uint8 * passwd, uint8 passwdlen)
{
   ...
   {
      printf("PAP\n\r"); // in case of adsl normally supports PAP.
