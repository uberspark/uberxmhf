...
void isl_handleintercept_exception_0D(u32 errorcode){
	...
	if(*((u8 *)pcpaddr) == 0xCB){
		...
		guest_RSP += 4;
 		...
 		//jmp sel:off
		guest_limbo_ip = *(u16 *)((u32)pcpaddr+1);
		guest_limbo_cs = *(u16 *)((u32)pcpaddr+3);
		printf("\n	INT 0D: target CS:EIP=0x%04x:0x%08x",
			guest_limbo_cs, guest_limbo_ip);
		guest_RIP += 5; 		
	}else{
...
#if defined(__CONF_SHADOW_PAGING__)
		ASSERT( !(guest_CR4 & CR4_PAE) ); //TODO: PAE shadowing
	...
	if(control_CR0_shadow & CR0_PG){
		...
		if(tofrom == VMX_CRX_ACCESS_FROM){
			...
			if ((guest_CR0 & CR0_WP) && (control_CR0_shadow & CR0_WP))
				...
				*((u32 *)islgprtable[gpr]) = guest_CR3;	//TODO: non-PAE CR0.WP=0 shadowing					
#else
			*((u32 *)islgprtable[gpr]) = guest_CR3;
			...
			if ((guest_CR0 & CR0_WP) && (control_CR0_shadow & CR0_WP))
				...
				guest_CR3 = *((u32 *)islgprtable[gpr]); //TODO: non-PAE CR0.WP=0 shadowing
#else
		...
		if( !(currentstate & GSTATE_PROTECTEDMODE_GDTR) ){
			...
			if(nextstate & GSTATE_PROTECTEDMODE_PG){
				...
				if ((guest_CR0 & CR0_WP) && (control_CR0_shadow & CR0_WP))
					...
					guest_CR3 = v86monitor_guest_reg_cr3; //TODO: non-PAE CR0.WP=0 shadowing
#else
