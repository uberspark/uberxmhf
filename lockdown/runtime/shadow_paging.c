...
/*
	AVL bits:
	_PAGE_BIT_UNUSED1 = dirty tracking flag, 0 = no tracking, 1 = tracking enabled 
	_PAGE_BIT_UNUSED2 = guest read/write bit value
	_PAGE_BIT_UNUSED3 = unused
	
	shadow: sPDPTE, sPDE, sPTE
	guest:	gPDPTE, gPDE, gPTE
	
	if(gPDPTE.present){
		sync_flags(sPDPTE, gPDPTE);
		add_pde(sPDPTE);
		
		if(gPDE.present){
			sync_flags(sPDE, gPDE);
			if(gPDE.BIG){
				copy_addr(sPDE, gPDE)
				//dirty tracking
				sPDE._PAGE_BIT_UNUSED1 = 1
				sPDE._PAGE_BIT_UNUSED2 = gPDE.READ/WRITE;
				sPDE.READ/WRITE = READ_ONLY;
			}else{
				add_pt(sPDE);
				
				if(sPTE.present){
					sync_flags(sPTE, gPTE);
					copy_addr(sPTE, gPTE);
					//dirty tracking
					sPTE._PAGE_BIT_UNUSED1 = 1
					sPTE._PAGE_BIT_UNUSED2 = gPTE.READ/WRITE;
					sPTE.READ/WRITE = READ_ONLY;
				}
			}	
		}
	}
...
/*	}else if (error_code & PFERR_WR_MASK){
	...
	/*
		TODO:
			1. get shadow page table entry for cr2 
			2. if present goto 3, else
					2.1 get guest page table entry for cr2 using shadow_guest_CR3
							update ACCESSED bits in PDEs and PTEs as we scan
					2.2 if not present, return VMX_EVENT_INJECT
					2.3 update shadow page table entry with guest page table entry
					2.4 set shadow page table entry to READ_ONLY, store original guest
							READ/WRITE bit in AVL bit of shadow entry and set wait for
							dirty bit in one of AVL bits
							return VMX_EVENT_CANCEL
			3.	if(write fault) then proceed, else goto 4
					3.1 if(wait for dirty bit is set in the shadow
							page table entry) then set corresponding guest entry dirty bit
							and unset wait-for dirty bit in shadow entry 
					3.2 if(original guest READ/WRITE bit is unset) 
								set shadow entry to READ/WRITE
								return VMX_EVENT_CANCEL
							else
								return VMX_EVENT_INJECT
			4. return VMX_EVENT_INJECT
	*/
#else
