...
#if defined (__CONF_SHADOW_PAGING__)
...
void isl_handleintercept_hlt(void){
	...
	printf("\nIntercept(HLT): Guest going to protected mode...");
	guest_nextstate = GSTATE_PROTECTEDMODE;
	//CD manipulation
	//v86monitor_guest_reg_cr0 &= (u32)~CR0_CD;
	//printf("\nCR0=0x%08x", v86monitor_guest_reg_cr0);
...
void isl_handleintercept_exception_0D(u32 errorcode){
	...
	if(*((u8 *)pcpaddr) == 0xCB){
		...
		guest_limbo_ip = *(u16 *)stackpaddr;
		guest_limbo_cs = *(u16 *)((u32)stackpaddr+2);
		//printf("\nguest_limbo cs:ip=0x%04x:0x%04x", guest_limbo_cs,
		//		guest_limbo_ip);
		guest_RSP += 4;
	}else{
		printf("\n%s: unrecognized opcode 0x%02x", __FUNCTION__, *((u8 *)pcpaddr));
		HALT();
	}
...
u32 isl_pgonoff_cr3=0;
u32 isl_pgonoff_cr4=0;

//u32 cr4paehandling=0; 	//1 if CR4.PAE needs to be handled when CR0.PG is
												//being set 
//u32 cr3handling=0;			//1 if CR3 needs to be loaded when CR0.PG is being set 
u32 guestcr3val_when_cr0_pg_off=0; //corresponding guest value to use when setting CR3
...
void handle_intercept_cr0access(u32 gpr, u32 tofrom){
	...
	//cr0value = value being moved into guest CR0
	//control_CR0_shadow = control_CR0_mask bit values currently seen by the guest
	//guest_CR0 = current value of guest CR0

#if defined(__CONF_SHADOW_PAGING__)
	//check for CR0.WP
	if( (cr0value & CR0_WP) && !(control_CR0_shadow & CR0_WP)){
		printf("\n0x%04x:0x%08x -> CR0.WP being set", (unsigned short)guest_CS_selector, (unsigned int)guest_RIP);
		control_CR0_shadow |= CR0_WP;
		//ok, we need to do a new context until we handle shadow paging when CR0.WP=0
		guest_CR3 = shadow_new_context(guest_CR3); 
	}else if( !(cr0value & CR0_WP) && (control_CR0_shadow & CR0_WP) ){
		printf("\n0x%04x:0x%08x -> CR0.WP being cleared", (unsigned short)guest_CS_selector, (unsigned int)guest_RIP);
		control_CR0_shadow &= ~(CR0_WP);
	}
#endif
	...
	if( (cr0value & CR0_PE) && 
...
#if defined(__CONF_SHADOW_PAGING__)
		if((guest_CR0 & CR0_WP) && (control_CR0_shadow & CR0_WP))
			guestcr3val_when_cr0_pg_off = shadow_guest_CR3;
		else
			guestcr3val_when_cr0_pg_off = guest_CR3; //TODO: PAE CR0.WP=0 shadowing
#else
		...
		//if(cr4paehandling){
			//now would be a good time to load guest CR4
		//	guest_CR4 = control_CR4_shadow;
		//	cr4paehandling=0;
		//}

		//if(cr3handling){
#if defined(__CONF_SHADOW_PAGING__)
		if(guest_CR4 & CR4_PAE){
			if((guest_CR0 & CR0_WP) && (control_CR0_shadow & CR0_WP))
				guest_CR3 = shadow_new_context(guestcr3val_when_cr0_pg_off);
			else
				guest_CR3 = guestcr3val_when_cr0_pg_off; //TODO: PAE CR0.WP=0 shadowing
		}else{
			guest_CR3 = guestcr3val_when_cr0_pg_off; //TODO: non-PAE shadowing
		}
#else
		...
		//if(cr3handling || cr4paehandling){
			u64 pdpte;
			unsigned offset = (guest_CR3 & (4096-1)) >> 5;
			u64 *pdpt = (u64 *) (u32)guest_CR3;

			printf("\nLoad PDPTE, Guest CR3=0x%08x, offset=%u", (unsigned int)guest_CR3, offset);
			pdpte = pdpt[offset+0];
			printf("\npdpte=0x%016llx", (unsigned long long)pdpte);
			guest_PDPTE0_high= (unsigned int)((u64)pdpte >> 32);
			guest_PDPTE0_full= (unsigned int)pdpte;
			pdpte = pdpt[offset+1];
			printf("\npdpte=0x%016llx", (unsigned long long)pdpte);
			guest_PDPTE1_high= (unsigned int)((u64)pdpte >> 32);
			guest_PDPTE1_full= (unsigned int)pdpte;
			pdpte = pdpt[offset+2];
			printf("\npdpte=0x%016llx", (unsigned long long)pdpte);
			guest_PDPTE2_high= (unsigned int)((u64)pdpte >> 32);
			guest_PDPTE2_full= (unsigned int)pdpte;
			pdpte = pdpt[offset+3];
			printf("\npdpte=0x%016llx", (unsigned long long)pdpte);
			guest_PDPTE3_high= (unsigned int)((u64)pdpte >> 32);
			guest_PDPTE3_full= (unsigned int)pdpte;
			
			printf("\nLoading guest PDPTEs in VMCS:");
			printf("\nPDPTE0: 0x%08x%08x", guest_PDPTE0_high, guest_PDPTE0_full);
			printf("\nPDPTE1: 0x%08x%08x", guest_PDPTE1_high, guest_PDPTE1_full);
			printf("\nPDPTE2: 0x%08x%08x", guest_PDPTE2_high, guest_PDPTE2_full);
			printf("\nPDPTE3: 0x%08x%08x", guest_PDPTE3_high, guest_PDPTE3_full);
			
			__invvpid(VMX_VPID_EXTENT_SINGLE_CONTEXT, 1, 0);			
		//}
#endif
		//isl_printgueststate();
		...
		//v86monitor_guest_reg_cr0 = control_CR0_shadow = cr0value;
...
void handle_intercept_cr3access(u32 gpr, u32 tofrom){
	...
	if(control_CR0_shadow & CR0_PG){
		...
		if(tofrom == VMX_CRX_ACCESS_FROM){
			...
			if(guest_CR4 & CR4_PAE){
				if ((guest_CR0 & CR0_WP) && (control_CR0_shadow & CR0_WP))
					*((u32 *)islgprtable[gpr]) = shadow_guest_CR3;
				else
					*((u32 *)islgprtable[gpr]) = guest_CR3;	//TODO: PAE CR0.WP=0 shadowing					
			}else{
				*((u32 *)islgprtable[gpr]) = guest_CR3;	//TODO: non-PAE shadowing		
			}
#else
			*((u32 *)islgprtable[gpr]) = guest_CR3;
#endif		
		//	printf("\n0x%04x:0x%08lx -> MOV reg=0x%08lx(%u), CR3=0x%08lx",
		//		(unsigned short)guest_CS_selector,
		//		(unsigned long)guest_RIP,
		//		(unsigned long)*((u32 *)islgprtable[gpr]),
		//		gpr,
		//		(unsigned long)guest_CR3);
			...
			if(guest_CR4 & CR4_PAE){
				if ((guest_CR0 & CR0_WP) && (control_CR0_shadow & CR0_WP))
					guest_CR3 = shadow_new_context( *((u32 *)islgprtable[gpr]) );
				else
					guest_CR3 = *((u32 *)islgprtable[gpr]); //TODO: PAE CR0.WP=0 shadowing
			}else{
				guest_CR3 = *((u32 *)islgprtable[gpr]); //TODO: non-PAE shadowing
			}
#else
			guest_CR3 = *((u32 *)islgprtable[gpr]);		
#endif
			//printf("\n0x%04x:0x%08lx -> MOV CR3=0x%08lx, reg=0x%08lx(%u)",
			//	(unsigned short)guest_CS_selector,
			//	(unsigned long)guest_RIP,
			//	(unsigned long)guest_CR3,
			//	(unsigned long)*((u32 *)islgprtable[gpr]),
	...
	if(tofrom == VMX_CRX_ACCESS_TO){
		if(guest_CR4 & CR4_PAE){
			...
			printf("\nCR3 load in PAE, Loading guest PDPTEs in VMCS:");
			printf("\nPDPTE0: 0x%08x%08x", guest_PDPTE0_high, guest_PDPTE0_full);
			printf("\nPDPTE1: 0x%08x%08x", guest_PDPTE1_high, guest_PDPTE1_full);
			printf("\nPDPTE2: 0x%08x%08x", guest_PDPTE2_high, guest_PDPTE2_full);
			printf("\nPDPTE3: 0x%08x%08x", guest_PDPTE3_high, guest_PDPTE3_full);
...
//VMX Inject #PF into guest
static inline int is_page_fault(u32 intr_info)
{
	return (intr_info & (INTR_INFO_INTR_TYPE_MASK | INTR_INFO_VECTOR_MASK |
			     INTR_INFO_VALID_MASK)) ==
		(INTR_TYPE_HW_EXCEPTION | PF_VECTOR | INTR_INFO_VALID_MASK);
}

static void vmx_inject_exception_PF(u32 addr, u32 error_code){
	u32 vect_info = (u32)info_IDT_vectoring_information;

	if (is_page_fault(vect_info)) {
		printf("%s: injecting #DF (addr=0x%08x, 0x%04x:0x%08x)", __FUNCTION__,
			(unsigned int)addr, (unsigned short)guest_CS_selector,
				(unsigned int)guest_RIP);

		control_VM_entry_exception_errorcode = 0;
		control_VM_entry_interruption_information = DF_VECTOR |
	...
	control_VM_entry_exception_errorcode = error_code;
	control_VM_entry_interruption_information = PF_VECTOR |
			     INTR_TYPE_HW_EXCEPTION |
			     INTR_INFO_DELIVER_CODE_MASK |
			     INTR_INFO_VALID_MASK;
		...
		case VMEXIT_EXCEPTION:{
			switch( ((u32)info_vmexit_interrupt_information & INTR_INFO_VECTOR_MASK) ){
				...
				case 0x0E:{	//(#PF, page-fault)
						u32 pf_error_code=0;
						u32 status;
						if((u32)info_vmexit_interrupt_information & INTR_INFO_DELIVER_CODE_MASK)
							pf_error_code = (u32)info_vmexit_interrupt_error_code;
						
						//guest_CR2 = info_exit_qualification;
						status=shadow_page_fault(info_exit_qualification, pf_error_code);		
				
						if(status == VMX_EVENT_INJECT){
							vmx_inject_exception_PF(info_exit_qualification, pf_error_code);
							guest_CR2 = info_exit_qualification;
						}else{
							//now we need to handle any nested events, e.g an external 
							//interrupt happened, but IDT page was not-present in SHADOW.
							//we would receive a PF intercept, sync SHADOW, cancel PF event,
							//but NEED to reinject the external interrupt
							vmx_inject_event(info_IDT_vectoring_information,	
...
#if defined(__CONF_SHADOW_PAGING__)
		case VMEXIT_INVLPG:{			
			u32 address=info_exit_qualification;
...
#if defined(__CONF_HARDWARE_PAGING)
	asm("xor	%%ebx, %%ebx \n"
			"mov	%0, %%ecx \n"
			"nxcap: \n"
			"rdmsr \n"
	...
	if(currentstate == GSTATE_DEAD){
		...
		//setup VMCS
		{
			...
			//guest_CR0 |= CR0_CD;
				...
				#if defined(__CONF_HARDWARE_PAGING__)
				  control_VMX_seccpu_based = msr0x480[ 11 ];
					control_VMX_cpu_based |= (1 << 31); //activate secondary processor controls
					control_VMX_seccpu_based |= (1 << 1); //enable EPT
					control_VMX_seccpu_based |= (1 << 5); //enable VPID
					control_vpid = 1; //VPID=0 is reserved for hypervisor
					
					vmx_setupEPT();
					//dumpmtrr();
					
					control_EPT_pointer_high = 0;
					control_EPT_pointer_full = (u32)__ept_pml4_table | 0x1E; //page walk of 4 and WB memory
					...
					//control_CR4_mask   = 0x00002000; //we dont care about other bits of CR4 except for VMX enable/disable bit 
	 				control_CR4_mask   = msr0x480[8] | CR4_PAE; //we dont care about other bits of CR4 except for VMX enable/disable bit
					control_CR4_shadow = CR4_VMXE; //let guest know we have VMX enabled
				
					control_VMX_cpu_based &= ~(1 << 15); //disable CR3 load exiting
					control_VMX_cpu_based &= ~(1 << 16); //disable CR3 store exiting
					
				#elif defined(__CONF_SHADOW_PAGING__)
					//TODO: ...
					control_pagefault_errorcode_mask  = 0x00000000;	//dont be concerned with 
					control_pagefault_errorcode_match = 0x00000000; //guest page-faults
					control_exception_bitmap = 0;
					...
					//control_CR4_mask   = msr0x480[8]; //we want to intercept access to these bits
	 				control_CR4_shadow = CR4_VMXE; //let guest know we have VMX enabled
				
					control_exception_bitmap |= (1UL << 14); //intercept all INT 14 (#PF)
					control_VMX_cpu_based |= (1 << 9);	//intercept INVLPG instruction
					control_CR0_mask 	|= (1 << 16);	//intercept CR0.WP access

					guest_CR0 |= CR0_WP;	//always set CR0.WP
					control_CR0_shadow &= ~(CR0_WP); //but make sure guest initially sees CR0.WP=0				
				#else
					#error Either CONF_HARDWARE_PAGING or CONF_SHADOW_PAGING must be set.
				#endif
		...
		//setup guest portion
		{
			//setup GDT
			{
					...
					//desc = (u64)(u32)__limbo_tss;
					//desc = ((desc & 0xFF000000)<<32)|((desc & 0x00FFFFFF)<<16);
					//desc |= ( 0x67 ) | (0x008BULL << 40);
					//gdt[guest_TR_selector >> 3 ] = desc;
			...
			//setup LDT
			{
				...
				//guest_LDTR_limit =0;
				//guest_LDTR_base=0;
				//guest_LDTR_access_rights = 0x10000; //unusable
			}			

			//setup TSS, only if guest does not already have a TR previously
			/*if(isl_guesthastr == 0){
				memset((void *)__limbo_tss, 0, __LIMBO_TSS_SIZE);
				guest_TR_base   = (u32)__limbo_tss;
				guest_TR_access_rights   = 0x8B;
				guest_TR_limit   = 0x67;
			}else{
				//set the guest TR values
			...
			//setup paging
			{
				/*if(!(nextstate & GSTATE_PROTECTEDMODE_PG) ){
					printf("\nGuest did not enable paging, using limbo pages");				
					guest_CR3 = (u32)__runtime_v86_pagedir;
					guest_CR4 = v86monitor_guest_reg_cr4;
					guest_CR4 |= control_CR4_mask;
					guest_CR0 = v86monitor_guest_reg_cr0;
					guest_CR0 |= control_CR0_mask;
				}else{
	...
	// END if(currentstate == GSTATE_REALMODE)
		...
		if( !(currentstate & GSTATE_PROTECTEDMODE_GDTR) ){
			...
			guest_ES_selector = 0;
				...
				guest_RFLAGS &= ~((1<<3)|(1<<5)|(1<<15));	// reserved 0-bits
				guest_RFLAGS |= (1<<1);				// reserved 1-bits
				//guest_RFLAGS &= ~(1<<17);	// disable Virtual-8086 mode
				//guest_RFLAGS &= ~(3<<12);	// IOPL=0 
				//guest_RFLAGS &= ~(1<<9);	// IF disable 
				//guest_RFLAGS &= ~(1<<14);	// NT (Nested Task) disable
				guest_ES_base = 0;
			...
			//control_VMX_cpu_based |= (1<<31);	// activate secondary proc
			//control_VMX_seccpu_based |= (1 << 2); //activate descriptor ins exiting
			...
			if(nextstate & GSTATE_PROTECTEDMODE_PG){
				...
				if(guest_CR4 & CR4_PAE){
					if ((guest_CR0 & CR0_WP) && (control_CR0_shadow & CR0_WP))
						guest_CR3 = shadow_new_context(v86monitor_guest_reg_cr3);
					else
						guest_CR3 = v86monitor_guest_reg_cr3; //TODO: PAE CR0.WP=0 shadowing
				}else{
					guest_CR3 = v86monitor_guest_reg_cr3; //TODO: non-PAE shadowing
				}
#else
					guest_CR3  = v86monitor_guest_reg_cr3;
#endif
			...
			//printf("\nTR info:");
			//printf("\nsel=0x%04x, base=0x%08lx, limit=0x%04x",
			//	(unsigned short)guest_TR_selector, (unsigned long)guest_TR_base,
			//		(unsigned short)guest_TR_limit);
			//printf("\nunder construction!");
			//HALT();
...
#if defined(__CONF_HARDWARE_PAGING__)
	{ 0x401E, &control_VMX_seccpu_based },
#endif
...
#if defined(__CONF_HARDWARE_PAGING__)
	{ 0x201A, &control_EPT_pointer_full }, 
	{ 0x201B, &control_EPT_pointer_high },
#endif
	//-------------------
	// Host-State fields
	//-------------------
...
#if defined(__CONF_HARDWARE_PAGING__)
	{ 0x280A, &guest_PDPTE0_full }, 
	{ 0x280B, &guest_PDPTE0_high },
	{ 0x280C, &guest_PDPTE1_full }, 
	{ 0x280D, &guest_PDPTE1_high },
	{ 0x280E, &guest_PDPTE2_full }, 
	{ 0x280F, &guest_PDPTE2_high },
	{ 0x2810, &guest_PDPTE3_full }, 
	{ 0x2811, &guest_PDPTE3_high },
#endif
...
//------------------------------------------------------------------------------
		...
		/*ASSERT( (nextstate == GSTATE_PROTECTEDMODE | GSTATE_PROTECTEDMODE_GDTR) ||
						(nextstate == GSTATE_PROTECTEDMODE | GSTATE_PROTECTEDMODE_GDTR |
													GSTATE_PROTECTEDMODE_IDTR)	||
						(nextstate == GSTATE_PROTECTEDMODE | GSTATE_PROTECTEDMODE_GDTR
													GSTATE_PROTECTEDMODE_PG) ||
						(nextstate == GSTATE_PROTECTEDMODE | GSTATE_PROTECTEDMODE_GDTR |
													GSTATE_PROTECTEDMODE_IDTR | GSTATE_PROTECTEDMODE_PG)							
					);*/
